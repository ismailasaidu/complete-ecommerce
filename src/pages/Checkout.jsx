      ...
      <div className="px-14 sm:px-[20px] mt-[200px]  relative">
        ...
        <form onSubmit={handleSubmit}>
              ...
              ? cart.map((item) => {
                  return (
                    <div className="flex flex-row justify-between border-divider border-[1px] py-[5px] px-[250px] h-[30px]  md:px-[50px]   sm:px-[40px]">
                      ...
                      <p className="font-medium text-[14px]  text-darktext font-Arimo">
                      ${ Math.round((item.data.Price - (item.data.Discount / 100) * item.data.Price )* item.data.quantity)}
                      </p>
                    </div>
                  );
                })
          ...
          <div className="relative mt-[40px] justify-between border-divider border-[1px] px-[10px] py-[10px] h-[160px]   sm:h-[150px]">
            ...
            <button className="bg-secondary text-white mt-[20px] px-[10px]" style={{display:isSubmit? "none":"block"}} onClick={handleSubmit }>Click here</button>

            {isSubmit ? (
              <PaystackButton
           
                {...componentProps}
                className="bg-secondary sm:bg-blue absolute right-[20px] top-[80px]  h-[50px] w-[150px]   text-white sm:w-[100px] sm:h-[40px] sm:text-[12px] sm:right-[10px] sm:top-[100px]"
              />
            ):""}
          </div>
        </form>
